import { r as reactExports, j as jsxRuntimeExports, i as Field, af as Checkbox$1, k as Label, S as Span, H as Hint, M as Message, s as styled, u as useTranslation, ag as MediaInput, ah as SvgCreditCardStroke, ai as Datepicker, I as Input$1, aj as debounce, F as Field$1, L as Label$1, V as Hint$1, C as Combobox, O as Option, W as Message$1, ak as OptGroup, d as useToast, N as Notification, e as Title, f as Close, al as Textarea } from 'shared';
...
function EmptyValueOption() {
    const { t } = useTranslation();
    return (jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [jsxRuntimeExports.jsx(Span, { "aria-hidden": "true", children: "-" }), jsxRuntimeExports.jsx(Span, { hidden: true, children: t("ticket-fields.dropdown.empty-option", "Select an option") })] }));
}
...
function LookupField({ field, userId, organizationId, onChange, }) {
    ...
    const loadingOption = {
        name: t("ticket-fields.lookup-field.loading-options", "Loading items..."),
        id: "loading",
    };
    const noResultsOption = {
        name: t("ticket-fields.lookup-field.no-matches-found", "No matches found"),
        id: "no-results",
    };
...
const isCustomField = (field) => {
    return (field.name.startsWith("custom_field_") ||
        field.name.startsWith("request[custom_fields]"));
};
const systemType = [
    "subject",
    "description",
...
const TicketFields = ({ fields, baseLocale, hasAtMentions, userRole, userId, defaultOrganizationId, organizationField, brandId, handleChange, }) => {
    return (jsxRuntimeExports.jsx(jsxRuntimeExports.Fragment, { children: fields.map((field) => {
            switch (field.type) {
                case "text":
                case "integer":
                case "decimal":
...
export { DatePicker as D, EmptyValueOption as E, Input as I, TextArea as T, TicketFields as a, getCustomObjectKey as g, isCustomField as i, systemType as s };
