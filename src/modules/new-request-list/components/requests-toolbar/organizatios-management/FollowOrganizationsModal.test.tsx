...
(globalThis.fetch as jest.Mock) = jest.fn((url, options) => {
  ...
  if (
    ...
    if (body.organization_subscription.organization_id === 4) {
      // Simulate subscribe failure for subscribtion with id 4
      status = false;
    ...
    status = true;
    // Simulate unsubscribe failure for subscribtion with id 4
  ...
  render(
    <ThemeProvider>
      <ModalContainerProvider>
        <FollowOrganizationsModal {...props} />
      </ModalContainerProvider>
    </ThemeProvider>
  );
...
describe("Follow organizations modal", () => {
  ...
  describe("rendering", () => {
    test("it correctly sets the initial state of the checkboxes", async () => {
      ...
      expect(
        (screen.getByTestId("checkbox_1") as HTMLInputElement).checked
      ).toBe(true);
      expect(
        (screen.getByTestId("checkbox_2") as HTMLInputElement).checked
      ).toBe(true);
      expect(
        (screen.getByTestId("checkbox_3") as HTMLInputElement).checked
      ).toBe(false);
      expect(
        (screen.getByTestId("checkbox_4") as HTMLInputElement).checked
      ).toBe(false);
    });
  });
  ...
  describe("subscribing", () => {
    describe("when the request fails", () => {
      test("it renders the error messages", async () => {
        ...
        fireEvent.click(screen.getByTestId("checkbox_4"));
    ...
    describe("when the request is successful", () => {
      test("it sends the creation request", async () => {
        ...
        fireEvent.click(screen.getByTestId("checkbox_3"));
        fireEvent.click(screen.getByText("Save"));
        await act(() => Promise.resolve());
  ...
  describe("unsubscribing", () => {
    ...
    describe("when successful", () => {
      beforeEach(async () => {
        ...
        fireEvent.click(screen.getByTestId("checkbox_2"));
        fireEvent.click(screen.getByText("Save"));
        await act(() => Promise.resolve());
      });
