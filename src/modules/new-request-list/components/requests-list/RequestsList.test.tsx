...
describe("Rendering", () => {
  ...
  test("<List /> doesn't render 'Organizational requests' tab if there are no organizations", async () => {
  ...
  test("<List /> renders custom fields in filter modal", async () => {
    await renderComponent();
  ...
  test("<List /> renders dropdown custom field options in filter modal", async () => {
    await renderComponent();
  ...
  test("<List /> renders multiselect custom field options in filter modal", async () => {
    await renderComponent();
...
describe("Sorting", () => {
  describe("created at", () => {
    test("<List /> can sort on the column", async () => {
      await renderComponent();
    ...
    test("<List /> should change sorting order to descending when the order is ascending", async () => {
      await renderComponent({
        sort: { by: "created_at", order: "asc" },
      });
    ...
    test("<List /> should clear the sorting when the order is descending", async () => {
      await renderComponent({
        sort: { by: "created_at", order: "desc" },
      });
...
describe("Filter tabs", () => {
  test("<List /> can filter by organization using the organizations menu on the 'Organizational requests' tab", async () => {
    await renderComponent({
      selectedTab: { name: ORG_REQUESTS_TAB_NAME, organizationId: 1 },
    });

    fireEvent.click(screen.getByTestId("organizations-menu"));
    fireEvent.click(
  ...
  test("<List /> filters by the default organization on the 'Organizational requests' tab", async () => {
    await renderComponent();
