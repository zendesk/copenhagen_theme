...
const fetchCategoryId = async (subdomain) => {
  const response = await fetch(`https://${subdomain}.zendesk.com/api/v2/help_center/categories`);
  const data = await response.json();
  const categoryId = data?.categories[0]?.id;

  if (!categoryId) {
    throw new Error('No category found. Please make sure the account has at least one category.');
  }
...
const fetchSectionId = async (subdomain) => {
  const response = await fetch(`https://${subdomain}.zendesk.com/api/v2/help_center/sections`);
  const data = await response.json();
  const sectionId = data?.sections[0]?.id;

  if (!sectionId) {
    throw new Error('No section found. Please make sure the account has at least one section.');
  }
...
const fetchArticleId = async (subdomain) => {
  const response = await fetch(`https://${subdomain}.zendesk.com/api/v2/help_center/articles`);
  const data = await response.json();
  const articleId = data?.articles[0]?.id;

  if (!articleId) {
    throw new Error('No article found. Please make sure the account has at least one article.');
  }
...
const fetchTopicId = async (subdomain) => {
  const response = await fetch(`https://${subdomain}.zendesk.com/api/v2/community/topics`);
  const data = await response.json();
  const topicId = data?.topics[0]?.id;

  if (!topicId) {
    throw new Error('No community topic found. Please make sure the account has at least one community topic.');
  }
...
const fetchPostId = async (subdomain) => {
  const response = await fetch(`https://${subdomain}.zendesk.com/api/v2/community/posts`);
  const data = await response.json();
  const postId = data?.posts[0]?.id;

  if (!postId) {
    throw new Error('No community post found. Please make sure the account has at least one community post.');
  }
...
const fetchUserId = async (subdomain, email, password) => {
  const response = await fetch(`https://${subdomain}.zendesk.com/api/v2/users/me`, {
    headers: {
...
const fetchRequestId = async (subdomain, email, password) => {
  const response = await fetch(`https://${subdomain}.zendesk.com/api/v2/requests`, {
    headers: {
      authorization: `Basic ${Buffer.from(`${email}:${password}`, 'binary').toString('base64')}`,
    },
  });
...
const buildUrlsFromAPI = async ({ subdomain, email, password }) => {
  const [categoryId, sectionId, articleId, topicId, postId, userId, requestId] = await Promise.all([
