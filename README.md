...
> **Warning:** `yarn download-locales` fetches strings for the upstream `help_center_copenhagen_theme` package and **overwrites every file in `translations/*.json` in full**, including `en-us.json`. In this fork, custom setting keys (e.g. anything added only to `en-us.json` and not upstream) are **not** known to Zendesk's translation system and will be silently deleted if this script is ever run. Do not run `yarn download-locales` in this repo; add/update custom keys directly in `translations/en-us.json` (and other locale files) by hand instead.
